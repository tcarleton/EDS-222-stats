...
print(point_est)
```
This doesn't look like "no difference", and it's consistent with our prior that non-smoking mothers should have higher birthweight babies. But remember that to conduct a hypothesis test we need a measure of variability, not just a measure of the mean.
...
```{r}
n1 = ncbirths_complete_habit %>% filter(habit == "nonsmoker") %>% count()
n2 = ncbirths_complete_habit %>% filter(habit == "smoker") %>% count()
s1 = ncbirths_complete_habit %>% filter(habit == "nonsmoker") %>% summarize(sd(weight, na.rm = TRUE))
s2 = ncbirths_complete_habit %>% filter(habit == "smoker") %>% summarize(sd(weight, na.rm = TRUE))
...
```{r}
t.test(ncbirths_complete_habit$weight[ncbirths_complete_habit$habit=="nonsmoker"], ncbirths_complete_habit$weight[ncbirths_complete_habit$habit=="smoker"] )
```
...
```{r}
t.test(weight ~ habit, data = ncbirths_complete_habit, conf.level = 0.90)
```