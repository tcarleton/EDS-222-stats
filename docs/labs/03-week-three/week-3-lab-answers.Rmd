...
```{r}
# (i) the length of possum tail is associated with the length of a possum's body:
...
**Because the slope coefficient is $>1$, increasing tail length must be associated with increasing body length as well. To see this, imagine that we instead found that $\hat\beta_1=1$. This would imply that increasing tail length by 1cm would simply increase $\text{total length} = \text{body length} + \text{tail length}$ by 1cm, leaving body length unchanged. Because we have $\hat\beta_1 = 1.24 > 1$, body length must also be increasing as tail length increases.** 
...
```{r}
# (ii) the length of possum tail is associated with the length of a possum's head:
...
```{r}
# (i) the length of possum tail is associated with the total length of a possum:
ggplot(data = possum, aes(y = total_l, x = tail_l)) +
  geom_point() + 
