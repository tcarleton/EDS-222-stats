...
**Note!** The expression for $\hat\beta_0$ can be rearranged to show us that our regression line always passes through the sample mean of $x$ and $y$.
...
`summary()` then lets us see the regression results. How do we interpret these??
...
- **Slope**: Change in $y$ for a one unit change in $x$.
  + Here: On average, we expect to see ozone increase by 2.5 ppb for each 1 degree F increase in temperature.
