...
```{R, example: pdf, echo = F, fig.height = 3.5}
...
```{R, example: pdf 2, echo = F, fig.height = 3.5}
# Generate data for density's polygon
tmp <- tibble(x = seq(-4, 4, 0.01), y = dnorm(x))
tmp <- rbind(tmp, tibble(x = seq(4, -4, -0.01), y = 0))
# Plot it
...
```{R, example: skew, echo = F, fig.height = 3}
# Generate data for density's polygon
set.seed(1298)
...
```{R, example: bimodal, echo = F, fig.height = 3}
df_modes <- tibble(
  uni   = rchisq(100, 6),
...
```{R, example: pdf 3, echo = F, fig.height = 3.5}
# Generate data for density's polygon
set.seed(1298)
tmp <- tibble(x = seq(-4, 4, 0.01), y = dnorm(x))
tmp <- rbind(tmp, tibble(x = seq(4, -4, -0.01), y = 0))
...
In a **population**, the mean is defined as: $$\mathrm{E}[X]=\mu=\int_x xf(x)dx$$
...
```{R, example: mean, echo = F,  fig.height = 3, fig.with=5.5}
# Generate data for density's polygon
set.seed(1298)
samp <- as.data.frame(rnorm(100))
  colnames(samp) = "x"
...
```{R, example: median, echo = F,  fig.height = 3, fig.with=5.5}
# Generate data for density's polygon
set.seed(1298)
samp <- as.data.frame(rnorm(100))
  colnames(samp) = "x"
...
```{R, example: median2, echo = F, fig.height = 3, fig.width=5.5}
# Generate data for density's polygon
set.seed(1298)
samp <- as.data.frame(rlnorm(1000))
  colnames(samp) = "x"
...
# Plot it
ggplot(data = samp, aes(x=x)) +
  geom_histogram(fill="grey85", bins=80) +
  geom_hline(yintercept = 0, color = "black") +
  theme_simple + ggtitle("sample (n=100)") + 
  geom_vline(xintercept=mn, color="black", linetype="solid") +
...
```{R, example: mode, echo = F, fig.height = 3.5, fig.width=6}
# Generate data for density's polygon
set.seed(1298)
samp <- as.data.frame(rlnorm(1000))
  colnames(samp) = "x"
samp <- samp %>% mutate(x = round(x,digits = 3))
mn = mean(samp$x)
md = median(samp$x)
...
> A: Lots of math to prove it (see [here](https://www.khanacademy.org/math/ap-statistics/summarizing-quantitative-data-ap/more-standard-deviation/v/review-and-intuition-why-we-divide-by-n-1-for-the-unbiased-sample-variance)), but trust me that your calculation of $s^2$ will be a biased estimate of $\sigma^2$ if you do not divide by $n-1$!
...
In the population: $$SD(X) = \sqrt{\mathrm{E}[(X-\mu)^2]} = \sigma = \sqrt{\int_{\mathrm R} (x-\mu)^2f(x)dx}$$
...
```{r, out.width = "75%", echo=FALSE, fig.align='center', fig.cap="Quartiles of the normal distribution"}
...
- At 24.5 lbs, my son's weight is the 81st percentile of U.S. male babies of 13 months old $\rightarrow$ 81% of American male 13 month olds are lighter than my son
- Your GRE score is in the 90th percentile of tests taken this year $\rightarrow$ 90% of GRE scores this year were lower than your score
...
```{R, scatter2, echo = F, fig.height=4}
ggplot(data = pop_df, aes(x = x, y = y)) +
geom_point(color = "darkslategray", size = 6) +
  xlab("Number of extreme heat days") +
  ylab("Number of violent crimes") +
